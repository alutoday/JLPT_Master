/**
 * Test Attempt Card Component
 * Full-featured card for history and test attempts pages
 */
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <span className={`flex-none px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide ${
              ...
              {attempt.is_completed ? '✓ Hoàn thành' : '⏱ Đang làm'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-400 mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <span>
                {new Date(attempt.started_at).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })} • {new Date(attempt.started_at).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>
            {attempt.is_completed && attempt.completed_at && (
              <>
                ...
                <span>
                  {new Date(attempt.completed_at).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })} • {new Date(attempt.completed_at).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
                </span>
              </>
            )}
          </div>
          ...
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-gray-100 dark:bg-slate-800 text-[11px] font-semibold text-gray-700 dark:text-gray-300">
            ...
            <span className="text-gray-500 dark:text-gray-500">sections</span>
          </div>
        </div>
