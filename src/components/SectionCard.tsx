...
export function SectionCard({ section, index, onClick, mode = 'detail' }: SectionCardProps) {
  const getSectionIcon = (sectionName: string, index: number) => {
  ...
  const getCTAText = () => {
    if (mode === 'detail') return 'Bắt đầu';
    if (section.status === 'COMPLETED') return 'Xem lại';
    if (section.status === 'NOT_STARTED') return 'Bắt đầu';
    return 'Tiếp tục';
  };
          ...
          {mode === 'attempt' && section.status === 'COMPLETED' ? (
            <>
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-medium text-gray-600 dark:text-gray-400">Điểm số</span>
              ...
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-gray-600 dark:text-gray-400 font-medium">Đúng</span>
            ...
            <>
              {section.question_count !== undefined && (
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-gray-600 dark:text-gray-400 font-medium">Câu hỏi</span>
                  <span className="font-bold text-gray-900 dark:text-white">
